<?php
// backend/app/Domain/ValueObjects/YouTubeUrl.php
...
final class YouTubeUrl
{
    ...
    public static function fromString(string $url): self
    {
        ...
        if (empty($url)) {
            throw new InvalidYouTubeUrlException('URL do YouTube não pode estar vazia');
        }

        $id = self::extractVideoId($url);
        
        if (!$id) {
            throw new InvalidYouTubeUrlException('URL do YouTube inválida: ' . $url);
        }
    ...
    public static function fromId(string $id): self
    {
        if (!self::isValidVideoId($id)) {
            throw new InvalidYouTubeUrlException('ID do YouTube inválido: ' . $id);
        }
